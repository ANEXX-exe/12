...
class QuizBot(commands.Bot):
    ...
    async def on_ready(self): print(f"✅ {self.user}"); await self.change_presence(activity=discord.Game(name="مسابقات 🎯"))
...
@bot.tree.command(name="لوحة_الصدارة")
async def lb(inter:discord.Interaction):
    top=top_scores()
    if not top: await inter.response.send_message("لا نقاط"); return
...
if __name__=="__main__":
    ...
    if not token: raise RuntimeError("ضع التوكن في .env")
    bot.run(token)
